...
export const _createFormula = (formulaStr, args, options = {}) => {
    ...
    if (!allowedPattern.test(formulaStr)) {
        console.error(`graphlyjs error: (101 unauthorized characters in formula "${formulaStr}")`);
        return null;
    }
    ...
    const transpile = (formula, mode) => {
        ...
        const parsePrimary = () => {
            ...
            if (t === '(') { 
                ...
                if (consume() !== ')') {
                    console.error(`graphlyjs error: (102 missing closing parenthesis in formula)`);
                }
                return `(${e})`; 
            }
            if (funcNames.includes(t)) {
                ...
                if (consume() !== ')') {
                    console.error(`graphlyjs error: (103 missing closing parenthesis after function "${t}")`);
                }
    ...
    try {
        ...
        console.error(`graphlyjs error: (104 failed to create formula function: ${e.message})`);
        return null; 
    }
};
