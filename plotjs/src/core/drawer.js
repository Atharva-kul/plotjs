...
export const _drawGraph = (points, ctx, options = {}) => {
    ...
    if (!ctx) {
        console.error("graphlyjs error: (401 2D context is missing in _drawGraph)");
        return;
    }
...
export const _drawGraphWebGL = (points, canvas, options = {}) => {
    ...
    if (!gl) {
        console.error("graphlyjs error: (402 WebGL context could not be initialized in _drawGraphWebGL)");
        return;
    }
    const { lineColor = 'white', lineWidth = 2 } = options;
    ...
    if (!pInfo) {
        ...
        const create = (t, s) => { 
            ...
            if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
                console.error(`graphlyjs error: (403 WebGL shader compilation failed: ${gl.getShaderInfoLog(sh)})`);
            }
            return sh; 
        };
        ...
        if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
            console.error(`graphlyjs error: (404 WebGL program linking failed: ${gl.getProgramInfoLog(prog)})`);
        }
...
export const _drawGraphGPUEvaluated = (glConfig, canvas, options = {}) => {
    const gl = canvas.getContext('webgl', { antialias: true, powerPreference: 'high-performance' });
    if (!gl) {
        console.error("graphlyjs error: (405 WebGL context could not be initialized in _drawGraphGPUEvaluated)");
        return;
    }
    ...
    if (!pInfo) {
        const create = (t, s) => { 
            const sh = gl.createShader(t); gl.shaderSource(sh,s); gl.compileShader(sh); 
            if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
                console.error(`graphlyjs error: (406 WebGL shader compilation failed in GPUEvaluated: ${gl.getShaderInfoLog(sh)})`);
            }
            return sh; 
        };
        const prog = gl.createProgram();
        ...
        gl.linkProgram(prog);
        if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
            console.error(`graphlyjs error: (407 WebGL program linking failed in GPUEvaluated: ${gl.getProgramInfoLog(prog)})`);
        }
        pInfo = { 
